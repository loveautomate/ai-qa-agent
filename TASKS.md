...
- **Branches:** `example/saucedemo-e2e`, `example/petstore-api`, `examples/prd-scenarios` (see `BRANCHING.md`).
- **Playwright projects:** `seed`, `e2e`, `api` in `playwright.config.ts` (JSON + HTML reporters; `test-results/report.json` drives Markdown).
- **`src/reporting/generate-report.mjs`:** templates `reports/summary.md` from JSON output (`npm run report:md`, or `npm run test:report` after a full run).
